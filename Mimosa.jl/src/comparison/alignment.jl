...
const ORIENTATIONS = (Orientation("++", 0), Orientation("+-", 1), Orientation("-+", 2), Orientation("--", 3))

function orientation_rank(label::AbstractString)
    for o in ORIENTATIONS
        o.label == label && return o.rank
    end
    throw(ArgumentError("invalid orientation label: $label."))
end
...
"""
function align_motif_matrices(query::AbstractMatrix{T}, target::AbstractMatrix{T}, metric::AbstractColumnMetric) where {T<:AbstractFloat}
...
(`++`, `+-`, `-+`, `--`).
"""
function score_motif_candidates(q_fwd::AbstractMatrix{T}, q_rev::AbstractMatrix{T}, t_fwd::AbstractMatrix{T}, t_rev::AbstractMatrix{T}, metric::AbstractColumnMetric) where {T<:AbstractFloat}
    candidates = Vector{MotifCandidate}(undef, 4)
    pairs = ((ORIENTATIONS[1], q_fwd, t_fwd),
