...
function _print_usage(io::IO)
    println(io, "usage: mimosa motif --query <path> --target <path> [--metric pcc|ed|cosine]")
    println(io, "       [--background 0.25] [--query-index 0] [--target-index 0]")
end

function _parse_args(args::Vector{String})
    kwargs = Dict{Symbol,Any}(:metric => "pcc", :background => 0.25, :query_index => 0, :target_index => 0)
    i = 1
    query = nothing
    target = nothing
    while i <= length(args)
...
function main(args::Vector{String}=ARGS)::Int
    ...
    try
        query_model = readmodel(query_path; index=kwargs[:query_index], background=kwargs[:background])
        target_model = readmodel(target_path; index=kwargs[:target_index], background=kwargs[:background])
